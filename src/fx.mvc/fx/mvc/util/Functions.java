...
final class Functions {
    ...
    private static final ClassValue<MethodHandles.Lookup> privateLookup = new ClassValue<>() {
        ...
        protected MethodHandles.Lookup computeValue(Class<?> type) {
            try { return MethodHandles.privateLookupIn(type,LOOKUP); }
            catch (Exception e) { return LOOKUP; }
        }
    };
    ...
    static MethodHandle methodHandle(Class<?> type, Method method) {
        try { return privateLookup.get(type).unreflect(method); }
        catch (Throwable e) { return uncheck(e); }
    }

    static MethodHandle constructorHandle(Class<?> type, Constructor<?> constructor) {
        try { return privateLookup.get(type).unreflectConstructor(constructor); }
        catch (Throwable e) { return uncheck(e); }
    }
    ...
    static <T> T callStatic(Class<?> refc, String name, Class<?> rtype, Object... args) {
        try {
            ...
            return (T) privateLookup.get(refc)
                .findStatic(refc,name,methodType).invokeWithArguments(args);
        }
        catch (Throwable e) { return uncheck(e); }
    }
    ...
    static <T> T callVirtual(Object obj, Method method, Object... args) {
        try {
            return (T) privateLookup.get(obj.getClass())
                .unreflect(method).invokeWithArguments(arguments(obj,args));
        }
        catch (Throwable e) { return uncheck(e); }
    }
